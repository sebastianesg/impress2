...
<div class="main-menu menu-fixed {{ $configData['theme'] === 'dark' || $configData['theme'] === 'semi-dark' ? 'menu-dark' : 'menu-light' }} menu-accordion menu-shadow" data-scroll-to-active="true">
    <div class="navbar-header">
        <ul class="nav navbar-nav flex-row">
            <li class="nav-item me-auto">
                <a class="navbar-brand" href="{{ url(env('APP_CMS').'/') }}">
                    <span class="brand-logo">
                        @include('cms/panels/logo')
                    </span>
                </a>
            </li>
    ...
    <div class="main-menu-content">
        ...
        @if (isset($menuData[0]))
            ...
            @if (in_array(Auth::user()->rol->name, $menu->roles))
            ...
            <li class="nav-item {{ $custom_classes }} {{ Route::currentRouteName() === $menu->slug ? 'active' : '' }}">
                <a href="{{ isset($menu->url) ? url(env('APP_CMS').'/'.$menu->url) : 'javascript:void(0)' }}" class="d-flex align-items-center" target="{{ isset($menu->newTab) ? '_blank' : '_self' }}">
                ...
                @if (isset($menu->submenu)) @include('cms/panels/submenu', ['menu' => $menu->submenu]) @endif
            </li>
            @endif
            @endif
            @endforeach
