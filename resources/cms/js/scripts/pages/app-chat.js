...
function getMessages(){
    $(".chat-navbar .avatar img").attr('src', actualLi.find(".avatar img").attr('src'));
    $(".chat-navbar h6").empty().append(actualLi.find("h5").text());
    $.post(messagesUrl, {uid : actualLi.data('id'), '_token' : $("input[name='_token']").val()}, function(data){
        $('.user-chats').find('.chats').empty().append(data);
...
function enterChat(source) {
    ...
    var fd = new FormData($('.chat-app-form')[0]);
    var files = $('#attach-doc')[0].files;
    if (/\S/.test(message) || files.length > 0) {
        $.ajax({
            url: sendMessageUrl,
            type: 'post',
            data: fd,
