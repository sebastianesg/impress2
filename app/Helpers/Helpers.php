...
class Helpers
{
    ...
    public static function getCatalogSelect($name, $selected, $t, $st = 0, $null = null, $req = null, $extraClass = ''){
	    $catalogo = Catalog::where('type', $t)->where('subtype', $st)->get();
	    $select = '<select class="form-control '.$extraClass.'" id="'.str_ireplace('[]', '', $name).'" '.($req ? 'required' : '').' name="'.$name.'">';
	    if ($null) $select .= "<option value=''>".$null."</option>";
	    foreach ($catalogo as $cat){
            $select .= '<option value="'.$cat->id.'"';
            if ($cat->id == $selected)$select .= ' selected="selected" ';
